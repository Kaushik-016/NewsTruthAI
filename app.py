...
def home():
    ...
    if form.generate.data:
    ...
    elif form.fetch_url.data:
        url = str(form.url.data or '').strip()
        if url:
            try:
                downloaded = requests.get(url, timeout=10)
                downloaded.raise_for_status()
                article_text = extract(downloaded.text) or ''
                form.original_text.data = article_text.strip()[:10000]
            except Exception:
                form.original_text.data = ''
        return render_template('home.html', form=form, output=False)
        ...
        if len(str(form.original_text.data)) > 10:
            ...
            # optional news API corroboration
            api_key = os.environ.get('NEWSAPI_KEY')
            if api_key:
                try:
                    q = output.get('preprocessed', '')[:128]
                    r = requests.get('https://newsapi.org/v2/everything', params={'q': q, 'pageSize': 3, 'language': 'en', 'apiKey': api_key}, timeout=10)
                    if r.ok:
